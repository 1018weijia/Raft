...
const (
	PullConfigInterval            = time.Millisecond * 100 // 获取config的间隔
	PullShardsInterval            = time.Millisecond * 200 // 获取shard的间隔
	WaitCmdTimeOut                = time.Millisecond * 500 // 等待命令执行的超时时间
	CallPeerFetchShardDataTimeOut = time.Millisecond * 500 // 请求shard的超时时间
	CallPeerCleanShardDataTimeOut = time.Millisecond * 500 // 清除shard的超时时间
...
type ShardKV struct {
	mu           sync.Mutex
	me           int // 记录自己的id
	rf           *raft.Raft // 用于和raft交互
	applyCh      chan raft.ApplyMsg
	make_end     func(string) *labrpc.ClientEnd
	gid          int // 记录自己的gid
	ctrlers      []*labrpc.ClientEnd // 记录所有的分片控制器
...
func (kv *ShardKV) unlock(msg string) {
	...
	kv.lockMsg = ""
	kv.mu.Unlock()
	if duration > MaxLockTime { //打印锁的时间
...
func (kv *ShardKV) pullConfig() {
	for {
		...
		case <-kv.stopCh:
			return
		case <-kv.pullConfigTimer.C: // 当定时器到期后，获取config
			//只有leader才能获取
			...
			kv.lock("pullconfig")
			lastNum := kv.config.Num // 记录当前的config序号
			kv.log("pull config,last: %d", lastNum)
			kv.unlock("pullconfig")

			config := kv.scc.Query(lastNum + 1) // 获取下一个config
			if config.Num == lastNum+1 {
				...
				if len(kv.inputShards) == 0 && kv.config.Num+1 == config.Num {
					...
					kv.unlock("pullconfig")
					//请求该命令
					kv.rf.Start(config.Copy()) // 通过raft发送config命令，调整shard的分配
				} else {
					kv.unlock("pullconfig")
				}
			}
			kv.pullConfigTimer.Reset(PullConfigInterval) // 重置定时器
		}
	}
}

// 该函数在计时器到期后，自动执行下面的三个函数
...
func StartServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int, gid int, ctrlers []*labrpc.ClientEnd, make_end func(string) *labrpc.ClientEnd) *ShardKV {
	...
	config := shardctrler.Config{
		Num:    0, // 初始化为0
		Shards: [shardctrler.NShards]int{}, //设置分片被分配到的组
		Groups: map[int][]string{}, // 设置组的信息
	}
	kv.config = config
	kv.oldConfig = config
