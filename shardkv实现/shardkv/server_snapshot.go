...
func (kv *ShardKV) saveSnapshot(logIndex int) {
	...
	if kv.maxraftstate == -1 || kv.persister.RaftStateSize() < kv.maxraftstate {
		return // 如果maxraftstate为-1，表示不用进行日志压缩；如果当前状态日志小于最大忍受值，就不压缩，返回
	}

	//生成快照数据，即对各类信息进行编码，都不为空时，进行压缩
	w := new(bytes.Buffer)
	...
	data := w.Bytes()
	kv.rf.Snapshot(logIndex, data) // 设置logIndex,和data进行绑定
}
...
func (kv *ShardKV) readPersist(isInit bool, snapshotTerm, snapshotIndex int, data []byte) {
	if data == nil || len(data) < 1 { // 这个data失效，直接返回
		return
	}
